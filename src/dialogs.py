...
class ChoiceWindow(object):
    def __init__(self, master, choices, title='Selection', text='Select item:', selected=0):
        ...
        self._window.bind('<MouseWheel>', lambda e: self._list.yview(tk.SCROLL,
...
def askForOpen(master, title, type=('Finite State Machine', '*.fsm')):
    return tkFileDialog.askopenfilename(parent=master, title=title, filetypes=[('All Files', '*.*'), type], defaultextension=type[1][-4:])

def askForSave(master, title, type=('Finite State Machine', '*.fsm')):
    return tkFileDialog.asksaveasfilename(parent=master, title=title, filetypes=[('All Files', '*.*'), type], defaultextension=type[1][-4:])
